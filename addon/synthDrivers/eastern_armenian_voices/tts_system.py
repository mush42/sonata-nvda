...
from .const import *
from .helpers import import_bundled_library, LIB_DIRECTORY, VOICES_DIRECTORY
...
class SonataVoice:
    ...
    def load(self):
        ...
        self.phonemizer = PhoneticEngine()
    ...
    async def synthesize(self, text, rate, volume, pitch, sentence_silence_ms):
        text = self.phonemizer.process(text)
...
class SonataTextToSpeechSystem:
    ...
    def load_piper_voices_from_nvda_config_dir(cls):
        return sorted(
            cls.load_voices_from_directory(VOICES_DIRECTORY),
