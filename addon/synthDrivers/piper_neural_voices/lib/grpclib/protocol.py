...
class Buffer:
    ...
    def unacked_size(self) -> int:
        return sum(
            self._unacked.get_nowait().ack_size for _ in range(self._unacked.qsize())
        )
...
class Connection:
    ...
    def _is_need_send_ping(self) -> bool:
        ...
        if (
            ...
            and self.ping_count_in_sequence
            >= self._config._http2_max_pings_without_data
        ):
            return False

        if (
            self.last_ping_sent is not None
            and time.monotonic() - self.last_ping_sent
...
class Stream:
        ...
        while True:
            ...
            try:
                self._h2_connection.send_headers(
                    stream_id, headers, end_stream=end_stream
                )
    ...
    async def send_headers(
        ...
        self._h2_connection.send_headers(self.id, headers, end_stream=end_stream)
        self._transport.write(self._h2_connection.data_to_send())
        self.connection.headers_send_process()
    ...
    async def send_data(self, data: bytes, end_stream: bool = False) -> None:
        ...
        while True:
            ...
            if f_pos == f_last:
                self._h2_connection.send_data(self.id, f_chunk, end_stream=end_stream)
                self._transport.write(self._h2_connection.data_to_send())
...
class EventsProcessor:
    ...
    def process_stream_reset(self, event: StreamReset) -> None:
        ...
        if stream is not None:
            if event.remote_reset:
                msg = "Stream reset by remote party, error_code: {}".format(
                    event.error_code
                )
            else:
                msg = "Protocol error"
    ...
    def process_connection_terminated(
        ...
        self.close(
            reason=(
                "Received GOAWAY frame, closing connection; error_code: {}".format(
                    event.error_code
                )
            )
        )
    ...
    def connection_made(self, transport: BaseTransport) -> None:
        ...
        if stream_delta:
            h2_conn.update_settings(
                {
                    SettingCodes.INITIAL_WINDOW_SIZE: self.config.http2_stream_window_size,
                }
            )
