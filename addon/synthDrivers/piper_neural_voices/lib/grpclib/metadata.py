...
USER_AGENT = "grpc-python-grpclib/{lib_ver} ({sys}; {py}/{py_ver})".format(
    lib_ver=__version__,
    sys=platform.system(),
...
class Deadline:
    ...
    def from_headers(cls, headers: _Headers) -> Optional["Deadline"]:
        timeout = min(
            map(decode_timeout, (v for k, v in headers if k == "grpc-timeout")),
            default=None,
        )
...
def encode_metadata(metadata: _MetadataLike) -> _Headers:
    ...
    for key, value in metadata:
        if key in _SPECIAL or key.startswith("grpc-") or not _KEY_RE.fullmatch(key):
            raise ValueError("Invalid metadata key: {!r}".format(key))
        if key.endswith("-bin"):
            if not isinstance(value, bytes):
                raise TypeError(
                    "Invalid metadata value type, bytes expected: " "{!r}".format(value)
                )
            result.append((key, encode_bin_value(value).decode("ascii")))
        else:
            if not isinstance(value, str):
                raise TypeError(
                    "Invalid metadata value type, str expected: " "{!r}".format(value)
                )
