...
class _EventMeta(type):
    def __new__(mcs, name, bases, params):  # type: ignore
        ...
        params["__readonly__"] = frozenset(
            name for name in annotations if name not in payload
        )
...
class _DispatchMeta(type):
    def __new__(mcs, name, bases, params):  # type: ignore
        dispatch_methods = dict(
            chain.from_iterable(
                getattr(base, "__dispatch_methods__", {}).items() for base in bases
            )
        )
        for key, value in params.items():
            dispatches = getattr(value, "__dispatches__", None)
            if dispatches is not None:
                assert isinstance(dispatches, type) and issubclass(
                    dispatches, _Event
                ), dispatches
...
class _DispatchServerEvents(_DispatchCommonEvents):
    ...
    async def send_initial_metadata(
        ...
        return await self.__dispatch__(
            SendInitialMetadata(  # type: ignore
                metadata=metadata,
            )
        )
    ...
    async def send_trailing_metadata(
        ...
        *,
        ...
        return await self.__dispatch__(
            SendTrailingMetadata(  # type: ignore
                metadata=metadata,
                status=status,
...
class _DispatchChannelEvents(_DispatchCommonEvents):
    ...
    async def recv_initial_metadata(
        ...
        return await self.__dispatch__(
            RecvInitialMetadata(  # type: ignore
                metadata=metadata,
            )
        )
    ...
    async def recv_trailing_metadata(
        ...
        *,
        ...
        return await self.__dispatch__(
            RecvTrailingMetadata(  # type: ignore
                metadata=metadata,
                status=status,
                status_message=status_message,
                status_details=status_details,
