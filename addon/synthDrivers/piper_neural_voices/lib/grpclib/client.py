...
class Stream(StreamIterator[_RecvType], Generic[_SendType, _RecvType]):
    ...
    async def send_request(self, *, end: bool = False) -> None:
        ...
        if end and not self._cardinality.client_streaming:
            raise ProtocolError(
                "Unary request requires a message to be sent "
    ...
    async def send_message(
        ...
        *,
        ...
        with self._wrapper:
            ...
            await send_message(
                self._stream, self._codec, message, self._send_type, end=end_stream
            )
    ...
    async def end(self) -> None:
        ...
        if not self._cardinality.client_streaming:
            if not self._send_message_done:
                raise ProtocolError(
                    "Unary request requires a single message " "to be sent"
                )
            else:
    ...
    def _raise_for_status(self, headers_map: Dict[str, str]) -> None:
        ...
        if status is not None and status != _H2_OK:
            ...
            raise GRPCError(grpc_status, "Received :status = {!r}".format(status))
    ...
    def _raise_for_content_type(self, headers_map: Dict[str, str]) -> None:
        ...
        if (
            ...
            raise GRPCError(
                Status.UNKNOWN, "Invalid content-type: {!r}".format(content_type)
            )
    ...
    def _process_grpc_status(
        ...
        try:
            ...
            raise GRPCError(
                Status.UNKNOWN, ("Invalid grpc-status: {!r}".format(grpc_status))
            )
        else:
    ...
    async def recv_trailing_metadata(self) -> None:
        ...
        """
        if not self._end_done and not (  # explicit end
            not self._cardinality.client_streaming  # implicit end
        ...
        if not self._recv_initial_metadata_done:
            raise ProtocolError(
                "Initial metadata was not received before "
    ...
    async def _maybe_finish(self) -> None:
        if not self._cancel_done and not self._stream._transport.is_closing():
            if not self._recv_initial_metadata_done:
                await self.recv_initial_metadata()
...
class Channel:
    ...
    def __init__(
        ...
        *,
        ...
        ssl: Union[None, bool, "_ssl.SSLContext", "_ssl.DefaultVerifyPaths"] = None,
        ...
        if loop:
            warnings.warn(
                "The loop argument is deprecated and scheduled "
    ...
    def __repr__(self) -> str:
        return "Channel({!r}, {!r}, ..., path={!r})".format(
    ...
    def _get_default_ssl_context(
        self,
        *,
        verify_paths: Optional["_ssl.DefaultVerifyPaths"] = None,
    ) -> "_ssl.SSLContext":
