...
class Stream(StreamIterator[_RecvType], Generic[_RecvType, _SendType]):
    ...
    async def send_trailing_metadata(
        ...
        *,
        ...
        if (
            ...
            raise ProtocolError(
                "Unary response with OK status requires " "a single message to be sent"
            )
        ...
        if status_message is not None:
            headers.append(("grpc-message", encode_grpc_message(status_message)))
        if status_details is not None and self._status_details_codec is not None:
    ...
    async def __aexit__(
        ...
        if exc_val is not None:
            ...
            status_details = None
            protocol_error = (
                "Unary response with OK status requires "
                "a single message to be sent: {!r}".format(self._method_name)
            )
        else:
            status = Status.OK
            status_message = None
...
async def request_handler(
    ...
    try:
        ...
        if content_type is None:
            await _abort(_stream, 415, Status.UNKNOWN, "Missing content-type header")
            return
        ...
        if (
            ...
            await _abort(
                _stream, 415, Status.UNKNOWN, "Unacceptable content-type header"
            )
            return
        ...
        if method is None:
            await _abort(_stream, 200, Status.UNIMPLEMENTED, "Method not found")
            return

        try:
            deadline = Deadline.from_headers(headers)
        except ValueError:
            await _abort(_stream, 200, Status.UNKNOWN, "Invalid grpc-timeout header")
            return
        ...
        *,
        ...
        if path is not None:
            self._server = await self._loop.create_unix_server(
                self._protocol_factory, path, sock=sock, backlog=backlog, ssl=ssl
            )
